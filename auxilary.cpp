...
*/
Mat getRangesAux(const Mat & data)
{
                         ...
                         Mat & classesTest)
{
    ...
    do 
    {
        printf("File name = ");
        scanf("%s", &fileName);
        //char *realFilename = strcat("../",fileName);
        FileStorage fs;
...
void readDatasetFromFileForKMeans(Mat & points)
{
    char fileName[maxFileNameLen];
    do
    {
...
int BoostPrediction::predict(const cv::Mat & sample, cv::ml::StatModel & model)
{
    int prediction = 0;
